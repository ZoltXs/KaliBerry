...
kaliberryconfig() {
    ...
    dialog --colors --title "KaliBerry Config" --backtitle "KaliBerry Config" \
        --infobox "Configuring Debian 11 for Raspberry Pi Zero 2 W..." 3 40
    sleep 1
...
colorberrydisplay() {
    ...
    show_progress_continue "cd sharp-drm-driver" "Changing to jdi-drm-rpi directory"
...
terminalmode() {
    ...
    show_progress_continue "sudo rm -f /boot/cmdline.txt" "Removing current cmdline.txt"
    show_progress_continue "echo 'console=serial0,115200 console=tty1 root=PARTUUID=c08eeb37-02 rootfstype=ext4 fsck.repair=yes rootwait net.ifnames=0 console=tty2 fbcon=font:VGA10x8 fbcon=map:10 quiet loglevel=0' | sudo tee /boot/cmdline.txt" "Creating new cmdline.txt"
...
guimode() {
    ...
    show_progress_continue "sudo apt-get install -y kali-desktop-xfce" "Installing Kali desktop environment"
    ...
    dialog --colors --title "GUI Mode" --backtitle "KaliBerry Config" \
        --infobox "Returning to Kali graphical environment" 3 40
    sleep 2
