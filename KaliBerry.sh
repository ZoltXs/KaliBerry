...
manual_make_install() {
    ...
    echo "25" | dialog --colors --title "$message" --backtitle "KaliBerry Config" \
        --gauge "Preparing environment..." 7 40 0
    ...
    echo "50" | dialog --colors --title "$message" --backtitle "KaliBerry Config" \
        --gauge "Running make install..." 7 40 0
    ...
    echo "75" | dialog --colors --title "$message" --backtitle "KaliBerry Config" \
        --gauge "Finalizing installation..." 7 40 0
...
kaliberryconfig() {
    cd /home/kali/KaliBerry > /dev/null 2>&1 || {
        display_error "Could not access directory"
        return 1
    }
    
    dialog --colors --title "KaliBerry Config" --backtitle "KaliBerry Config" \
        --infobox "Configuring for Pi Zero 2 W..." 3 35
    sleep 1
    
    show_progress "sudo cp /etc/apt/sources.list /etc/apt/backupsoucelist.txt" "Backing up configuration"
...
EOF'" "Configuring repositories"

    show_progress "sudo rm /etc/apt/sources.list.d/re4son.list" "Removing Kali list"
    
    show_progress "wget -qO- https://archive.raspberrypi.org/debian/raspberrypi.gpg.key | sudo apt-key add -" "Importing Pi key"
    
    show_progress "wget -qO- https://ftp-master.debian.org/keys/archive-key-11.asc | sudo apt-key add -" "Importing Debian key"
    
    show_progress "sudo apt-get install -y raspberrypi-kernel" "Installing Pi kernel"
    
    show_progress "sudo apt-get install -y raspberrypi-kernel-headers" "Installing headers"
    
    show_progress "sudo apt-get install -y build-essential git wget" "Installing dev tools"
...
colorberrydisplay() {
    ...
    show_progress_continue "sudo cp -r jdi-drm-rpi /var/tmp/" "Copying driver files"
    show_progress_continue "cd /" "Going to root"
    show_progress_continue "cd /var/tmp/jdi-drm-rpi" "Changing directory"
    show_progress_continue "cd /var/tmp/jdi-drm-rpi && sudo make" "Compiling driver"
    manual_make_install "Installing driver"
    show_progress_continue "sudo mkdir -p /home/kali/sbin" "Creating sbin dir"
    show_progress_continue "sudo cp /home/kali/KaliBerry/back.py /home/kali/sbin/" "Copying back.py"
    show_progress_continue "sudo chmod +x /home/kali/sbin/back.py" "Making executable"
    show_progress_continue "(sudo crontab -l 2>/dev/null; echo '@reboot sleep 5; /home/kali/sbin/back.py &') | sudo crontab -" "Setting crontab"
    show_progress_continue "echo 'dtoverlay=sharp-drm' | sudo tee -a /boot/config.txt" "Adding overlay"
...
rm /tmp/bashrc_append.txt" "Configuring .bashrc"
    show_progress_continue "sudo apt-get install -y python3-pip" "Installing pip"
...
colorberrykbd() {
    ...
    show_progress_continue "cd beepberry-keyboard-driver" "Changing directory"
    show_progress_continue "cd /home/kali/KaliBerry/beepberry-keyboard-driver && sudo make" "Compiling driver"
    show_progress_continue "cd /home/kali/KaliBerry/beepberry-keyboard-driver && sudo make install" "Installing driver"

    dialog --colors --title "Colorberry-KBD" --backtitle "KaliBerry Config" \
        --infobox "Installation completed" 3 25
    sleep 2
...
terminalmode() {
    show_progress_continue "sudo systemctl set-default multi-user.target" "Disabling desktop"
    show_progress_continue "sudo systemctl disable gdm3" "Disabling gdm3"
    ...
    show_progress_continue "echo "keys" | sudo tee /sys/module/beepy_kbd/parameters/touch_as > /dev/null"
    show_progress_continue "sudo rm -f /boot/cmdline.txt" "Removing cmdline.txt"
    show_progress_continue "echo 'console=serial0,115200 console=tty1 root=PARTUUID=c08eeb37-02 rootfstype=ext4 fsck.repair=yes rootwait net.ifnames=0 console=tty2 fbcon=font:VGA10x8 fbcon=map:10 quiet loglevel=0' | sudo tee /boot/cmdline.txt" "Creating cmdline.txt"
...
guimode() {
    show_progress_continue "sudo systemctl set-default graphical.target" "Enabling desktop"
    show_progress_continue "sudo apt-get install -y kali-desktop-xfce" "Installing desktop"
    show_progress_continue "sudo apt-get install -y xorg" "Installing X server"
    show_progress_continue "sudo systemctl enable lightdm" "Enabling lightdm"
    show_progress_continue "sudo systemctl enable gdm3" "Enabling gdm3"
...
update() {
    show_progress_continue "sudo rm /etc/apt/sources.list" "Removing sources.list"
    show_progress_continue "sudo mv /etc/apt/backupsoucelist.txt /etc/apt/sources.list" "Restoring config"
    show_progress_continue "sudo apt-key del ED444FF07D8D0BF6" "Removing keys"
    show_progress_continue "sudo rm -rf /etc/apt/trusted.gpg.d/*" "Cleaning keys"
    show_progress_continue "wget -qO- https://archive.kali.org/archive-key.asc | sudo apt-key add -" "Importing Kali key"
    show_progress_continue "wget -qO- https://www.kali.org/archive-key.asc | sudo apt-key add -" "Importing key"
    show_progress_continue "sudo apt update" "Updating packages"
    show_progress_continue "sudo apt upgrade -y" "Upgrading system"

    dialog --colors --title "Update" --backtitle "KaliBerry Config" \
        --msgbox "Update completed\n\nNever update Kernel Headers" 7 35
    sleep 3
